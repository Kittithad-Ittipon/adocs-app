...
function hostCard(host) {
  ...
  <div class="host-card group relative overflow-hidden rounded-2xl border border-zinc-200 bg-white p-5 transition hover:border-zinc-300 hover:bg-zinc-50 dark:border-white/8 dark:bg-white/[0.03] dark:hover:border-white/16 dark:hover:bg-white/[0.05]" data-search="${searchKey}">
    ...
    <div class="mt-4 flex items-center gap-2 font-mono text-[12px] text-zinc-500">
      ...
      <span>${escapeHtml(host.forwardHost)}:${escapeHtml(String(host.forwardPort || 80))}</span>
    </div>
  ...
  <div class="relative mx-auto max-w-5xl px-6 pb-24 pt-10">
    ...
    <nav class="flex items-center justify-between">
      ...
      <div class="flex items-center gap-2">
        ...
        <!-- ปุ่ม Refresh เพิ่มใหม่ -->
        <button id="refresh-btn" class="flex h-8 w-8 items-center justify-center rounded-full border border-zinc-200 bg-white text-zinc-600 transition hover:bg-zinc-50 dark:border-white/10 dark:bg-white/[0.03] dark:text-zinc-300 dark:hover:bg-white/[0.06]" title="โหลดข้อมูลใหม่">
          <svg id="refresh-icon" class="h-4 w-4 text-zinc-500 dark:text-zinc-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
        ...
        <div id="host-count-badge" class="rounded-full border border-zinc-200 bg-white px-3 py-1 text-xs text-zinc-600 dark:border-white/10 dark:bg-white/[0.03] dark:text-zinc-400">
    ...
    <!-- Setup guide (เนื้อหาเดิมของคุณครบถ้วน) -->
    <section class="mt-16">
      ...
      <ol class="mt-5 space-y-3">
        <li class="rounded-2xl border border-zinc-200 bg-white p-5 dark:border-white/8 dark:bg-white/[0.03]">
          ...
          <div class="mt-3 space-y-2 pl-8 text-xs leading-relaxed text-zinc-600 dark:text-zinc-400">
            <p>กด <kbd class="rounded bg-zinc-200 px-1.5 py-0.5 font-mono text-[11px] text-zinc-700 dark:bg-white/10 dark:text-zinc-300">Win + R</kbd> พิมพ์ <span class="font-mono text-zinc-800 dark:text-zinc-300">ncpa.cpl</span> แล้ว Enter (หรือเข้า Control Panel → Network and Internet → Network and Sharing Center → Change adapter settings)</p>
        ...
        <li class="rounded-2xl border border-zinc-200 bg-white p-5 dark:border-white/8 dark:bg-white/[0.03]">
          ...
          <p class="mt-3 pl-8 text-xs leading-relaxed text-zinc-600 dark:text-zinc-400">
            เลื่อนลงไปดูรายการทั้งหมดด้านล่าง แต่ละ host จะบอกชื่อ certificate ที่ใช้จริงกำกับไว้ คลิกชื่อ domain เพื่อเปิดเว็บนั้นในแท็บใหม่
          </p>
        </li>
      </ol>
    </section>
    ...
    <section class="mt-16">
      <div class="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div class="flex items-center gap-3">
          ...
          <span id="host-total-count" class="font-mono text-[11px] text-zinc-400 dark:text-zinc-600">${hosts.length} total</span>
        </div>
        <input
          id="host-search"
          type="text"
    ...
    <script>
      (function () {
        ...
        var totalCount = document.getElementById("host-total-count");
        ...
        if (input) input.addEventListener("input", filterHosts);

        // 3. Auto-Refresh Logic (Client-Side Rendering)
        ${hostCard.toString()} 
        ${statusDot.toString()} 
        ${escapeHtml.toString()}
        ${domainHref.toString()}
        ...
        async function fetchHostsData() {
          try {
            ...
            const newHosts = await res.json();
            
            if (newHosts.length === 0) {
              grid.innerHTML = '<div class="col-span-full rounded-2xl border border-zinc-200 bg-white p-6 text-center text-sm text-zinc-500 dark:border-white/8 dark:bg-white/[0.03]">ยังไม่มี proxy host ที่ตั้งค่าไว้</div>';
            } else {
              grid.innerHTML = newHosts.map(hostCard).join('');
            }

            const enabledCount = newHosts.filter(h => h.enabled).length;
            if (badge) badge.innerText = enabledCount + " host" + (enabledCount === 1 ? "" : "s") + " online";
            if (totalCount) totalCount.innerText = newHosts.length + " total";

            filterHosts();
            
          } catch (err) {
            console.error("Failed to auto-refresh hosts:", err);
          } finally {
            if (refreshIcon) setTimeout(() => refreshIcon.classList.remove("animate-spin"), 500);
          }
        }

        if (refreshBtn) refreshBtn.addEventListener("click", fetchHostsData);

        // ดึงข้อมูลใหม่ทุกๆ 30 วินาที
        setInterval(fetchHostsData, 30000); 
      })();
    </script>
    ...
    <footer class="mt-20 flex items-center justify-between border-t border-zinc-200 pt-6 text-[11px] text-zinc-500 dark:border-white/5 dark:text-zinc-600">
      ...
      <span class="font-mono">${new Date().toLocaleString("sv-SE", { timeZone: "Asia/Bangkok" })} (GMT+7)</span>
    </footer>
